...
# Your palette (hex order preserved). Names are examples — not matched to data.
# Developed-Roads #999999, fallow/crop/pasture yellows, forest greens, water, floodplain purples, etc.
evt_seed_hex <- c(
  "#999999", "#EFEEC5", "#E8E5A4", "#CDC90F", "#9A970B",
  "#70B65F", "#C4BBD5", "#D7EDF7", "#92C484", "#9B89B4",
  "#00703C", "#5E3D7F", "#3CABB5", "#351150", "#00381E"
)

evt_nm <- sort(unique(stats::na.omit(as.character(hexs$evt_name))))
n_evt <- length(evt_nm)

if (n_evt == 0L) {
  pal_evt <- NULL
} else {
  evt_hex <- stats::setNames(rep_len(evt_seed_hex, n_evt), evt_nm)
  pal_evt <- leaflet::colorFactor(
    palette = unname(evt_hex[evt_nm]),
    domain  = evt_nm
  )
}

# Optional: warn if classes recycle (same color, different EVT)
if (n_evt > length(evt_seed_hex)) {
  message(
    "EVT classes (", n_evt, ") exceed palette length (",
    length(evt_seed_hex), "); colors repeat in name order."
  )
}
...
to_hex_map <- function(ramp_tbl, no_data_hex = no_data_color) {
  ...
  hx <- grDevices::rgb(ramp_tbl$R, ramp_tbl$G, ramp_tbl$B, maxColorValue = 255)
  hx[vals == -9999L] <- no_data_hex
  names(hx) <- vals
  hx
}
...
hexs2 <- hexs %>%
  dplyr::mutate(
    ...
    evc_hex = dplyr::case_when(
      is.na(evc_value) ~ no_data_color,
      evc_value %in% as.integer(names(evc_hex_map)) ~
        unname(evc_hex_map[as.character(evc_value)]),
      TRUE ~ no_data_color
    ),
    evh_hex = dplyr::case_when(
      is.na(evh_value) ~ no_data_color,
      evh_value %in% as.integer(names(evh_hex_map)) ~
        unname(evh_hex_map[as.character(evh_value)]),
      TRUE ~ no_data_color
    )
  )
...
if (is.null(pal_evt)) {
  lf <- lf %>% leaflet::addPolygons(
    fillColor = no_data_color,
    color = "#B0B0B8", weight = 1, opacity = 1, fillOpacity = 0.95,
    highlightOptions = leaflet::highlightOptions(
      weight = 2, color = "#333333", fillOpacity = 0.98, bringToFront = TRUE
    ),
    label = ~evt_name,
...
lf %>%
  leaflet::addPolygons(
    fillColor = ~ifelse(is.na(evc_value) | evc_value == -9999L, no_data_color, evc_hex),
    color = "#B0B0B8", weight = 1, opacity = 1, fillOpacity = 0.95,
    highlightOptions = leaflet::highlightOptions(
      weight = 2, color = "#333333", fillOpacity = 0.98, bringToFront = TRUE
    ),
  ...
  leaflet::addPolygons(
    fillColor = ~ifelse(is.na(evh_value) | evh_value == -9999L, no_data_color, evh_hex),
    color = "#B0B0B8", weight = 1, opacity = 1, fillOpacity = 0.95,
    highlightOptions = leaflet::highlightOptions(
      weight = 2, color = "#333333", fillOpacity = 0.98, bringToFront = TRUE
    ),
  ...
  leaflet::hideGroup("Existing Vegetation Cover") %>%
  leaflet::hideGroup("Existing Vegetation Height")
```
